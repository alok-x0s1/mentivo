...
const communitiesData = [
	...
	},
	{
		...
		description:
			"Master Python, pandas, NumPy, and machine learning basics",
	},
	{
...
const learningGoalsData = {
	"Modern Full Stack Next.js Course": [
		...
		},
		{
			...
			description:
				"Write type-safe, maintainable full-stack applications",
		},
		{
	...
	"Developer to Leader": [
		{
			...
			description:
				"Learn to guide technical decisions and mentor developers",
		},
		{
			title: "Code review best practices",
			description:
				"Conduct effective code reviews that improve team quality",
		},
		{
			title: "Team communication",
			description:
				"Master stakeholder communication and conflict resolution",
		},
		{
			title: "Project planning & estimation",
			description:
				"Break down projects and estimate timelines accurately",
		},
		{
	...
	"Ankita's Youtube Community": [
		...
		},
		{
			...
			description:
				"Navigate job search, interviews, and career advancement",
		},
		{
...
async function comprehensiveSeed() {
	...
	try {
		...
		for (const [userName, assignment] of Object.entries(
			...
			if (!community) {
				console.error(`Community not found: ${assignment.community}`);
				continue;
			}
		...
		const createdGoals = [];
		for (const [communityName, goals] of Object.entries(
			learningGoalsData,
		)) {
			const community = createdCommunities.find(
				(c) => c.name === communityName,
		...
		for (const [userName, assignment] of Object.entries(
			...
			if (!templateGoal) {
				console.error(`Goal not found: ${assignment.goal}`);
				continue;
			}
		...
		for (const [userName, communities] of Object.entries(
			...
			for (const [communityName, goalTitles] of Object.entries(
				communities,
			)) {
				const community = createdCommunities.find(
					(c) => c.name === communityName,
		...
		for (const conv of createdConversations) {
			...
			for (
				let i = 0;
				i < messageCount && i < messageTemplates.length;
				i++
			) {
				const template = messageTemplates[i];
				const senderId =
					template.sender === "user1" ? user1!.id : user2!.id;
