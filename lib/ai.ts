...
export const aiMatchUsers = async (
	...
	try {
		...
		//get all the other members in the same community (not including the current user) and their learning goals
		const members = await getMembersInCommunity(communityId, user.id);

		// get all the matches for the current user
