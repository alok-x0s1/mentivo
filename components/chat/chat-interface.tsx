...
export default function ChatInterface({ matchId }: { matchId: string }) {
	...
	const currentUser = {
		name:
			(clerkUser?.firstName + " " + clerkUser?.lastName).trim() ?? "You",
				...
				<Card className="h-150 flex flex-col">
					<CardHeader className="border-b">
						{!conversation ? (
							...
							<div className="flex items-center gap-3">
								<UserAvatar
									name={otherUser?.name ?? "Default User"}
				...
				<Card className="w-full">
					...
					<CardContent className="space-y-4">
						{summary === undefined ? (
							...
							<div className="text-sm py-8 text-muted-foreground text-center">
								No summary yet 🤖 <br />
