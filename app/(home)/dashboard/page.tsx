...
export default function DashboardPage() {
	...
	const { data: allMatches } = useQuery({
		...
		queryFn: async () => {
			...
			if (!res.ok) {
				throw new Error("Failed to fetch user's pending matches");
			}
	...
	const { data: learningGoals } = useQuery({
		...
		queryFn: async () => {
			...
			if (!res.ok) {
				throw new Error("Failed to fetch user's learning goals");
			}
			return res.json();
		},
	...
	return (
		<div className="page-wrapper">
			...
			<Card className="border-primary">
				<CardHeader>
					<CardTitle>
						🎉 You have {pendingMatchesData?.length || 0} new{" "}
			...
			<div className="grid gap-4 md:grid-cols-4">
				{isLoadingUserCommunities ? (
					...
					<ErrorCard
						message={
							(errorUserCommunities as Error)?.message ??
			...
			<div className="grid gap-6 lg:grid-cols-2">
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Conversations you&apos;re part of or have started
						</CardDescription>
					</CardHeader>
					<CardContent>
						{isLoadingMatches ? (
