...
import { Bot, LockIcon } from "lucide-react";
...
export default function CommunitiesPage() {
	...
	}, [communities?.length, selectedCommunity, communities]);
					...
					{isLoadingCommunities ? (
						Array.from({ length: 4 }).map((_, i) => (
							<div key={i} className="w-full">
								<div className="h-9 w-full rounded-md bg-foreground/20 animate-pulse" />
							</div>
						))
					) : errorCommunities ? (
						<ErrorCard
							message={
								errorCommunities.message ||
				...
				<CardHeader>
					<div className="flex gap-2 mb-4">
						<Button
							...
							variant={
								activeTab === "goals" ? "default" : "outline"
							}
						>
							My Goals
						</Button>
						<Button
							...
							variant={
								activeTab === "matches" ? "default" : "outline"
							}
						>
							<Bot className="size-4 mr-1" /> Find Partners with
							AI
						</Button>
					</div>
					<CardTitle>
					...
					{activeTab === "goals" ? (
						<div className="space-y-2">
							{isLoadingCommunityGoals ? (
								...
								<ErrorCard
									message={errorCommunityGoals.message}
								/>
							) : (
								<>
